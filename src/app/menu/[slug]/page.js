...
export default function MenuPage() {
  ...
  useEffect(() => {
    const fetchMenuData = async () => {
      ...
      try {
        ...
        const grouped = data.dishes.reduce((acc, dish) => {
  ...
  useEffect(() => {
    if (restaurant && restaurant.themeColor) {
      document.documentElement.style.setProperty('--theme-color', restaurant.themeColor);
      // Créer une version plus claire de la couleur pour les fonds
      const lightColor = restaurant.themeColor + '1A'; // Ajoute de la transparence
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 py-8">
          <div className="text-center">
            {restaurant && restaurant.logo ? (
              <Image src={restaurant.logo} alt={`Logo de ${restaurant.restaurantName}`} width={80} height={80} className="rounded-2xl object-cover mx-auto mb-4 border-4 border-[var(--theme-color)] shadow-lg" />
            ) : (
              <div style={{borderColor: 'var(--theme-color)'}} className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-purple-400 to-pink-500 rounded-2xl mb-4 shadow-lg"><span className="text-2xl">🍴</span></div>
            )}
            <h1 style={{color: 'var(--theme-color)'}} className="text-5xl md:text-6xl font-bold bg-clip-text text-transparent mb-2 tracking-tight">
              {restaurant ? restaurant.restaurantName : 'Menu'}
            </h1>
            <div className="flex items-center justify-center space-x-2 mb-4"><div style={{backgroundColor: 'var(--theme-color)'}} className="w-2 h-2 rounded-full animate-pulse"></div><p className="text-gray-300 text-lg font-medium">Menu Gastronomique</p><div style={{backgroundColor: 'var(--theme-color)'}} className="w-2 h-2 rounded-full animate-pulse animation-delay-500"></div></div>
            <div style={{backgroundImage: `linear-gradient(to right, var(--theme-color), #f472b6)`}} className="w-24 h-1 rounded-full mx-auto"></div>
          </div>
        </div>
      </header>
      ...
      {Object.keys(dishes).length > 0 && (
        <nav className="relative z-10 bg-black/20 backdrop-blur-sm border-b border-white/5 sticky top-0">
          <div className="container mx-auto px-4 py-4"><div className="flex flex-wrap justify-center gap-2">{Object.keys(dishes).map((category) => (<button key={category} onClick={() => setSelectedCategory(category)} className={`flex items-center space-x-2 px-6 py-3 rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 ${selectedCategory === category ? 'text-white shadow-lg' : 'bg-white/10 text-gray-300 hover:bg-white/20 hover:text-white'}`} style={selectedCategory === category ? {backgroundColor: 'var(--theme-color)'} : {}}><span className="text-xl">{getCategoryIcon(category)}</span><span>{category}</span><span className="bg-white/20 px-2 py-1 rounded-full text-xs">{dishes[category].length}</span></button>))}</div></div>
        </nav>
      )}
          ...
          <div className="space-y-8">
            {Object.entries(dishes).filter(([category]) => !selectedCategory || category === selectedCategory).map(([category, items]) => (
              <section key={category} className="space-y-6">
                <div className="text-center mb-8"><div className="inline-flex items-center space-x-3 px-8 py-4 rounded-2xl shadow-xl border border-white/20" style={{background: `linear-gradient(to right, var(--theme-color-light), transparent)`}}><span className="text-4xl">{getCategoryIcon(category)}</span><h2 className="text-3xl font-bold bg-clip-text text-transparent" style={{color: 'var(--theme-color)'}}>{category}</h2></div></div>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {items.map((dish, index) => (
                    <div key={dish._id} className={`group bg-gradient-to-br from-white/90 to-gray-50/90 backdrop-blur-sm border border-white/20 rounded-3xl shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-500 hover:-translate-y-2 overflow-hidden ${!dish.isAvailable ? 'opacity-40 saturate-50' : ''}`} style={{animationDelay: `${index * 100}ms`}}>
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-4"><div className="flex-1"><div className="flex items-center space-x-2 mb-2"><div style={{backgroundColor: 'var(--theme-color)'}} className="w-3 h-3 rounded-full"></div><h3 className="text-xl font-bold text-gray-800 group-hover:text-gray-900 transition-colors">{dish.name}</h3></div>{dish.description && (<p className="text-gray-600 leading-relaxed mb-4 group-hover:text-gray-700 transition-colors">{dish.description}</p>)}</div></div>
                        <div className="flex items-center justify-between">
                          <div style={{backgroundColor: 'var(--theme-color)'}} className="px-4 py-2 text-white rounded-xl shadow-lg"><span className="text-2xl font-bold">{dish.price}</span><span className="text-sm ml-1">FCFA</span></div>
